...
public class DiceManager : MonoBehaviour
{
    ...
    public void addDice(int moreDice)
    {
        ...
        {
            int spawnValue = Random.Range(0, 6);
            GameObject spawnedDice = Instantiate(dices[spawnValue], diceInitialPosition[GameManager.Instance.playerManager.storedDice.Count].transform.position, Quaternion.identity);
            GameManager.Instance.playerManager.storedDice.Add(spawnedDice);
            GameManager.Instance.playerManager.numberOfDice += 1;
        }
    }
